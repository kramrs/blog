package com.kramrs.quartz.utils;

import com.kramrs.constant.ScheduleConstant;
import com.kramrs.entity.Task;
import com.kramrs.enums.TaskStatusEnum;
import com.kramrs.exception.ServiceException;
import com.kramrs.quartz.execution.QuartzDisallowConcurrentExecution;
import com.kramrs.quartz.execution.QuartzJobExecution;
import org.quartz.*;

import static com.kramrs.constant.CommonConstant.TRUE;
...
public class ScheduleUtils {
    ...
    public static void createScheduleJob(Scheduler scheduler, Task task) {
        try {
            ...
            CronTrigger trigger = TriggerBuilder.newTrigger().withIdentity(getTriggerKey(taskId, taskGroup)).withSchedule(cronScheduleBuilder).build();
            // 放入参数，运行时的方法可以获取
    ...
    public static CronScheduleBuilder handleCronScheduleMisfirePolicy(Task task, CronScheduleBuilder cb) throws ServiceException {
        switch (task.getMisfirePolicy()) {
            ...
            default:
                throw new ServiceException("The task misfire policy '" + task.getMisfirePolicy() + "' cannot be used in cron schedule tasks");
        }
    }
}
